...
TARGET="$TARGET" EXP_NAME="$EXP_NAME" RUNTIME_CFG="$RUNTIME_CFG" python3 - <<'EOF'
import os, yaml
...
# Namespace the shared intermediate files per target so concurrent array tasks never
# collide on the processed data CSV or the flat ar/midas/transformer pred files (the data
# and split are identical across targets, so this changes file NAMES only, not the numbers).
cfg["paths"]["data_processed_template"] = f"data/processed/long_format_fred_{tgt}_{{suffix}}.csv"
outs = cfg["paths"]["outputs"]
for k in ["transformer_preds", "ar_preds", "midas_preds"]:
    outs[k] = f"outputs/{k}_{tgt}_{{suffix}}.csv"
