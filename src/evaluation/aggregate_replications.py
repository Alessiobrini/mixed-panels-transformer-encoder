...
def make_latex(summary: pd.DataFrame) -> str:
    ...
    # rows: the mean (with best/second-best cell highlight) on top, and the SD in \footnotesize
    ...
    L = ["% Auto-generated by src/evaluation/aggregate_replications.py",
         ...
         "{\\footnotesize\\setlength{\\tabcolsep}{4pt}\\begin{tabular}{lccccccccc}", "\\toprule",
        ...
        for regime in REGIME_ORDER:
            for metric in METRICS:
                ...
                if best.get((regime, metric)) == model:
                    ...
                    m = f"\\second{{{m}}}"
                mean_cells.append(m)
                std_cells.append(f"{{\\footnotesize ({std:.4f})}}")
        L.append(f"{model}\n& " + " & ".join(mean_cells) + " \\\\")
