"""
Build per-stock long-format CSVs for the extended equity dataset (v2).

Replaces build_equity_long.py by sourcing daily data from CRSP + WRDS IID
(instead of VOLARE). Produces the same long-format output so all downstream
code (train.py, run_equity_pipeline.py, baselines, evaluation) works unchanged.
