...
def _sample_student_t(rng, size, cov, df):
    """Draw multivariate Student-t noise with unit variance scaling.

    The draw follows the Gaussian scale-mixture representation and rescales so
    that the resulting marginal covariance equals ``cov``. This keeps parity
    with the Gaussian branch while still producing heavy-tailed innovations.
    """
    ...
    gaussian = rng.normal(size=(size, p)) @ chol.T
    chi2 = rng.chisquare(df, size=size)
    # Standard Student-t has variance df/(df-2).  Rescale to unit variance so
    # that the caller-supplied covariance matrix remains the marginal
    # covariance of the innovations.
    scale = np.sqrt((df - 2) / chi2)
...
if __name__ == "__main__":
    ...
    noise_x = getattr(config.simulation, "noise_x", "student_t")
